...
...which, unfortunately, did not do anything. That is because `map()` actually returns a brand new array when it is done and so it `newUsers` is actually going to be an array of `Promises` so we can't really wait an array.
...
So, turns out a simple for loop would have solved all our issues from the start but they aren't the most ergonomic code blocks to look at ever:
